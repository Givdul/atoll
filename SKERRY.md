# Skerry Website/Demo Spec

## Product

Skerry is a native macOS menu-bar app that shows a Dynamic-Island-style session capsule for local coding agents. It receives lifecycle hooks through a local Unix socket, then displays only sessions that are actively running, need attention, or have just completed.

The app name is `Skerry`.

Primary source references:

- `README.md`
- `Sources/Skerry/AppDelegate.swift`
- `Sources/Skerry/IslandView.swift`
- `Sources/Skerry/IslandWindowController.swift`
- `Sources/Skerry/StatusMenuController.swift`
- `Sources/Skerry/SkerryIcon.swift`
- `Sources/Skerry/AgentGlyphView.swift`
- `Sources/SkerryCore/AgentHarness.swift`
...
- Captured from `/Applications/Skerry.app` after directly submitting normalized `started` events through its installed lifecycle CLI for Codex, Claude Code, Cursor Agent, OpenCode, and Pi.
...
- Captured from `/Applications/Skerry.app`; all five provider tiles are visible.
...
Skerry runs as an accessory app:
...
- A row captured from a regular macOS application opens that application without activating Skerry.
- Skerry first verifies the captured PID still has the same bundle identifier, then tries another running instance, then launches the installed application.
...
All dimensions derive from `IslandMetrics` in `Sources/Skerry/IslandView.swift`.
...
- Tooltip: `"Skerry"`.
- Disabled menu title: `"Skerry"`.
- Toggle: `"Show Island"`.
- Toggle: `"Test Mode"`.
- Command: `"Refresh Now"`, key equivalent `r`.
- Command: `"Quit Skerry"`, key equivalent `q`.
...
- Hooks deliver `started`, terminal, and optional attention events through `~/.skerry/lifecycle.sock`.
...
- Provider-specific repair changes only missing, stale, or partial Skerry-owned content, reruns checks in the same panel, and never treats static readiness alone as `Ready`.

Settings:

- File: `~/.skerry/config.json`.
...
Each supported harness can use Skerry's normalized `--lifecycle-event <harness> <kind>` bridge.

## Icons

The release app icon is the checked-in bundle asset:

- Source: `Bundle/Skerry.icns`
- Copied release path after build: `dist/Skerry.app/Contents/Resources/Skerry.icns`
- Installed release icon path: `/Applications/Skerry.app/Contents/Resources/Skerry.icns`

`SkerryIcon.swift` supplies the runtime application/status glyph used by AppKit; it does not generate the shipped `.icns` during release builds:

- Runtime glyph source: `Sources/Skerry/SkerryIcon.swift`
...
| --- | --- | --- |
| codex | Codex | `Sources/Skerry/Resources/AgentIcons/codex.svg` |
| claude | Claude Code | `Sources/Skerry/Resources/AgentIcons/claude.svg` |
| cursor | Cursor Agent | `Sources/Skerry/Resources/AgentIcons/cursor.svg` |
| opencode | OpenCode | `Sources/Skerry/Resources/AgentIcons/opencode.svg` |
| pi | Pi | `Sources/Skerry/Resources/AgentIcons/pi.svg` |
...
- `skerry`: custom stroked Skerry glyph.
