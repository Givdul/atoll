...
private enum AgentIconLibrary {
    private static let bundleName = "Topside_Topside.bundle"
...
private struct TopsideGlyph: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        for capsule in TopsideIcon.markRects(in: rect) {
            path.addRoundedRect(
                in: capsule,
                cornerSize: CGSize(width: capsule.height / 2, height: capsule.height / 2)
            )
        }
        return path
    }
