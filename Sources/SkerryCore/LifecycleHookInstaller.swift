...
public struct LifecycleHookInstaller {
    ...
    public enum Error: Swift.Error, LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .managedFileConflict(let url): "Cannot install Skerry's managed integration because \(url.path) already belongs to another extension or plugin."
            ...
            case .unreadablePiVersion(let minimum, let detail): "Pi \(minimum) or newer is required for Live Status, but Skerry could not verify it: \(detail)"
            ...
            case .configurationChanged(let url): "\(url.path) changed during setup. Skerry left it untouched; try again."
            }
        }
    }
    ...
    public init(
        ...
        executablePath: String = "/Applications/Skerry.app/Contents/MacOS/Skerry",
                ...
                if let index = results.indices.last,
                   case .failed = results[index].outcome {
                    ...
                    results[index] = RemovalResult(
                        ...
                        outcome: .failed("The integration was removed, but Skerry could not remove its unused bridge: \(error.localizedDescription)")
                    )
                }
            }
        }
    ...
    public func hasIntegration(for agent: AgentHarness) -> Bool {
        switch agent {
        case .pi:
            return ([piExtensionURL] + betaPiExtensionURLs).contains(where: managedFileIsOurs)
        case .opencode:
            return ([openCodePluginURL, legacyOpenCodePluginURL] + betaOpenCodePluginURLs)
            ...
            do {
                ...
                return readiness(requiring:
                    managedFileMatches(piExtensionSource, at: piExtensionURL)
                        && !betaPiExtensionURLs.contains(where: managedFileIsOurs)
                )
            } catch {
        ...
        if agent == .opencode {
            return readiness(requiring:
                managedFileMatches(openCodePluginSource, at: openCodePluginURL)
                    && !([legacyOpenCodePluginURL] + betaOpenCodePluginURLs)
                        .contains(where: managedFileIsOurs)
            )
        }
        do {
    ...
    private var bridgeURL: URL { homeDirectory.appendingPathComponent(".skerry/bin/skerry-hook") }
    private var betaBridgeURL: URL { homeDirectory.appendingPathComponent(".atoll/bin/atoll-hook") }
    ...
    private var piExtensionURL: URL { piConfigurationDirectory.appendingPathComponent("extensions/skerry.ts") }
    private var openCodePluginURL: URL { openCodeConfigurationDirectory.appendingPathComponent("plugins/skerry.js") }
    private var legacyOpenCodePluginURL: URL { openCodeConfigurationDirectory.appendingPathComponent("plugin/skerry.js") }
    private var betaPiExtensionURLs: [URL] {
        [piConfigurationDirectory.appendingPathComponent("extensions/atoll.ts")]
    }
    private var betaOpenCodePluginURLs: [URL] {
        [
    ...
    private var managedMarker: String { "Skerry Live Status managed integration" }
    private var betaManagedMarker: String { "Atoll Live Status managed integration" }
    ...
    private var openCodePluginSource: String {
        ...
        // \(managedMarker)
        const bridge = \(javaScriptString(bridgeURL.path));
        export const SkerryLiveStatus = async ({ directory }) => {
    ...
    private func writePiExtension() throws {
        let betaSnapshots = try ownedManagedFileSnapshots(at: betaPiExtensionURLs)
        try writeManagedFile(piExtensionSource, to: piExtensionURL)
        try removeUnchangedManagedFiles(betaSnapshots)
    }

    private func writeOpenCodePlugin() throws {
        let obsoleteSnapshots = try ownedManagedFileSnapshots(
            at: [legacyOpenCodePluginURL] + betaOpenCodePluginURLs
        )
    ...
    private func runCommand(executable: URL, arguments: [String]) throws -> CommandResult {
        let outputRoot = fileManager.temporaryDirectory
            .appendingPathComponent("SkerryLifecycleCommand-\(UUID().uuidString)")
    ...
    private func integrationState(for agent: AgentHarness) -> Diagnostic.Integration {
        ...
        do {
            ...
            case .pi:
                try requireSupportedPiVersion()
                if betaPiExtensionURLs.contains(where: managedFileIsOurs) {
                    return .outdated
                }
            ...
            case .opencode:
                if fileManager.fileExists(atPath: openCodePluginURL.path) {
                    guard managedFileIsOurs(at: openCodePluginURL) else {
                        return .unowned("The OpenCode plugin path belongs to another file. Skerry left it untouched.")
                    }
                    if ([legacyOpenCodePluginURL] + betaOpenCodePluginURLs)
                        .contains(where: managedFileIsOurs) {
                        return .outdated
                    }
    ...
    private func managedIntegrationState(expected: String, at url: URL) -> Diagnostic.Integration {
        ...
        guard managedFileIsOurs(at: url) else {
            return .unowned("\(url.path) belongs to another extension. Skerry left it untouched.")
        }
    ...
    private func shadowingState(for agent: AgentHarness) -> Diagnostic.Shadowing {
        if agent == .claude, managedClaudeSetting("allowManagedHooksOnly") == true {
            return .blocked("Managed Claude Code policy allows only administrator hooks, so it blocks Skerry's user hook.")
        }
        if agent == .codex,
           let source = try? String(contentsOf: codexRequirementsURL, encoding: .utf8),
           managedCodexHooksOnly(in: source) {
            return .blocked("Managed Codex policy allows only administrator hooks, so it blocks Skerry's user hook.")
        }
        return .notDetected
    }
    ...
    private func removeIntegration(for agent: AgentHarness) throws -> Bool {
        ...
        case .pi:
            return try removeManagedFiles(
                at: [piExtensionURL] + betaPiExtensionURLs,
                requiringOwnershipAt: [piExtensionURL]
            )
        case .opencode:
            return try removeManagedFiles(
                at: [openCodePluginURL, legacyOpenCodePluginURL] + betaOpenCodePluginURLs,
    ...
    private func managedFileIsOurs(_ contents: Data) -> Bool {
        ...
        guard let firstLine = source
            .split(separator: "\n", maxSplits: 1, omittingEmptySubsequences: false)
            .first else { return false }
        return firstLine == "// \(managedMarker)" || firstLine == "// \(betaManagedMarker)"
    }

    private func ownedCommands(for agent: AgentHarness) -> Set<String> {
        Set(LifecycleEventKind.allCases.flatMap {
            [
                hookCommand(harness: agent.rawValue, kind: $0.rawValue),
                betaHookCommand(harness: agent.rawValue, kind: $0.rawValue)
            ]
        })
    }
        ...
        Self.supportedAgents.contains { agent in
            ...
            case .opencode:
                return fileReferencesBridge(openCodePluginURL)
                    ...
                    || betaOpenCodePluginURLs.contains(where: fileReferencesBridge)
            case .pi:
                return fileReferencesBridge(piExtensionURL)
                    || betaPiExtensionURLs.contains(where: fileReferencesBridge)
            default:
    ...
    private func fileReferencesBridge(_ url: URL) -> Bool {
        ...
        return [bridgeURL.path, betaBridgeURL.path].contains {
            source.contains($0)
    ...
    private func jsonValueReferencesBridge(_ value: Any) -> Bool {
        if let string = value as? String {
            return string.contains(bridgeURL.path) || string.contains(betaBridgeURL.path)
        }
    ...
    private func backupSharedConfigurationIfNeeded(
        ...
        let pathDigest = SHA256.hash(data: Data(url.path.utf8))
            ...
            .joined()
        let directory = homeDirectory.appendingPathComponent(".skerry/backups/live-status", isDirectory: true)
    ...
    private func betaHookCommand(harness: String, kind: String) -> String {
        "\(shellQuote(betaBridgeURL.path)) \(harness) \(kind)"
    }
