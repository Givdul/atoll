...
public struct SkerrySettings: Codable, Sendable {
...
public final class SettingsStore: @unchecked Sendable {
    ...
    public init(homeDirectory: URL = FileManager.default.homeDirectoryForCurrentUser) {
        let directory = homeDirectory.appendingPathComponent(".skerry", isDirectory: true)
        self.directory = directory
        self.url = directory.appendingPathComponent("config.json")
        encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
    }

    public func load() -> SkerrySettings {
        try? PrivateStorage.ensureDirectory(at: directory)
        if FileManager.default.fileExists(atPath: url.path) {
            try? PrivateStorage.hardenFile(at: url)
        }
        guard let data = try? Data(contentsOf: url),
              let settings = try? decoder.decode(SkerrySettings.self, from: data) else {
            return SkerrySettings()
        }
        return settings
    }

    public func save(_ settings: SkerrySettings) {
