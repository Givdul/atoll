...
private final class LiveStatusSetupModel: ObservableObject {
    ...
    func status(for agent: AgentHarness) -> AgentDoctorStatus {
        ...
        guard let diagnostic = diagnostics[agent.rawValue] else {
            return .attention("Atoll could not check this provider.")
        }
        ...
        case .socketUnavailable:
            return .attention("Atoll's local socket is unavailable.")
        }
    }
    ...
    private var actions: some View {
        ...
        case .repairing:
            Button("Repairing and Rechecking…") {}
                ...
                .disabled(true)
        case .confirmingRemoval:
            Button("Remove Atoll from \(model.removableAgents.count) \(model.removableAgents.count == 1 ? "Provider" : "Providers")", action: model.remove)
...
private extension ProviderDiagnostic {
    var statusTitle: String {
        ...
        case .socketUnavailable: "Atoll socket unavailable"
    ...
    var guidance: String {
        switch health {
        case .agentNotFound:
            "Install \(agent.displayName), then check again. Atoll does not create provider configuration for an absent agent."
        case .integrationMissing:
            "Atoll can add its user-level integration without replacing existing valid settings."
        case .integrationOutdated:
            "Atoll can replace only its own stale or partial integration content."
        case .externalConfiguration(let detail), .shadowed(let detail):
            detail
        case .bridgeUnavailable:
            "Atoll can restore its private command bridge and required permissions."
        case .socketUnavailable:
            "Restart Atoll. The integration cannot deliver events while its local app socket is unavailable."
        case .runtimeUnverified:
            agent.activationGuidance
        case .ready(let date):
            "Atoll last accepted a valid local event \(date.formatted(date: .abbreviated, time: .shortened))."
        }
    }
...
private extension AgentHarness {
    var activationGuidance: String {
        switch self {
        case .codex:
            "In Codex, run /hooks, enable and trust the exact Atoll UserPromptSubmit and Stop commands, then send a new prompt."
        case .claude:
        ...
        case .atoll:
            "No external activation step applies to Atoll."
        }
    }
}
