...
private final class LiveStatusSetupModel: ObservableObject {
    ...
    func status(for agent: AgentHarness) -> AgentDoctorStatus {
        ...
        guard let diagnostic = diagnostics[agent.rawValue] else {
            return .attention("Skerry could not check this provider.")
        }
        ...
        case .socketUnavailable:
            return .attention("Skerry's local socket is unavailable.")
        }
    }
    ...
    private var actions: some View {
        ...
        case .repairing:
            Button("Repairing and Rechecking…") {}
                ...
                .disabled(true)
        case .confirmingRemoval:
            Button("Remove Skerry from \(model.removableAgents.count) \(model.removableAgents.count == 1 ? "Provider" : "Providers")", action: model.remove)
...
private struct LiveStatusMark: View {
    var body: some View {
        AgentGlyphView(harness: .skerry, glyphColor: .primary)
            .frame(width: 42, height: 42)
            .accessibilityLabel("Skerry")
    }
}
...
private extension ProviderDiagnostic {
    var statusTitle: String {
        ...
        case .socketUnavailable: "Skerry socket unavailable"
    ...
    var guidance: String {
        switch health {
        case .agentNotFound:
            "Install \(agent.displayName), then check again. Skerry does not create provider configuration for an absent agent."
        case .integrationMissing:
            "Skerry can add its user-level integration without replacing existing valid settings."
        case .integrationOutdated:
            "Skerry can replace only its own stale or partial integration content."
        case .externalConfiguration(let detail), .shadowed(let detail):
            detail
        case .bridgeUnavailable:
            "Skerry can restore its private command bridge and required permissions."
        case .socketUnavailable:
            "Restart Skerry. The integration cannot deliver events while its local app socket is unavailable."
        case .runtimeUnverified:
            agent.activationGuidance
        case .ready(let date):
            "Skerry last accepted a valid local event \(date.formatted(date: .abbreviated, time: .shortened))."
        }
    }
...
private extension AgentHarness {
    var activationGuidance: String {
        switch self {
        case .codex:
            "In Codex, run /hooks, enable and trust the exact Skerry UserPromptSubmit and Stop commands, then send a new prompt."
        case .claude:
        ...
        case .skerry:
            "No external activation step applies to Skerry."
        }
    }
}
