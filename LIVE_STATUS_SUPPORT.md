...
Atoll honors inherited absolute or home-relative custom user homes for `CODEX_HOME`, `CLAUDE_CONFIG_DIR`, `PI_CODING_AGENT_DIR`, and `OPENCODE_CONFIG_DIR`. Relative custom homes are deliberately not guessed by the GUI because their meaning depends on the launching shell's working directory. Project, system, and managed-policy layers are never modified.
