...
final class LifecycleRuntimeEvidenceStoreTests: XCTestCase {
    func testStoresOnlyProviderAndLatestLocalReceiptTime() throws {
        ...
        let file = home.appendingPathComponent(".skerry/lifecycle-runtime-evidence.json")
    ...
    func testRejectsInternalPresentationHarness() {
        ...
        XCTAssertFalse(store.record(provider: .skerry, receivedAt: Date()))
        XCTAssertNil(store.lastValidEvent(for: .skerry))
    }
    ...
    func testInvalidatesOnlyRequestedProvider() {
        ...
        XCTAssertTrue(store.invalidate(providers: [.codex, .skerry]))
    ...
    func testHardensExistingStorageOnLoadWithoutRewritingContents() throws {
        let home = temporaryHome()
        defer { try? FileManager.default.removeItem(at: home) }
        let root = home.appendingPathComponent(".skerry", isDirectory: true)
    ...
    private func temporaryHome() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("SkerryRuntimeEvidenceTests-\(UUID().uuidString)", isDirectory: true)
    }
