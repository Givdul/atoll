...
final class ReleaseScriptTests: XCTestCase {
    ...
    private func runReleaseScript(
        ...
        try writeExecutable(
            ...
            case "$*" in
              *appcast.xml*)
                printf 'network appcast\\n' >> "$SKERRY_TEST_SIDE_EFFECTS"
                printf '%s' "$SKERRY_TEST_APPCAST"
                ;;
              *latest-build.txt*)
                printf 'network ledger\\n' >> "$SKERRY_TEST_SIDE_EFFECTS"
                printf '%s' "$SKERRY_TEST_LEDGER"
                ;;
              *)
                exit 1
                ;;
        ...
        environment["SKERRY_TEST_APPCAST"] = appcast ?? Self.appcast(builds: [])
        environment["SKERRY_TEST_LEDGER"] = ledger
        environment["SKERRY_TEST_SIDE_EFFECTS"] = sideEffects.path
