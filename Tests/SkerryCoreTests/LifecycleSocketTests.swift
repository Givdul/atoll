...
final class LifecycleSocketTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("SkerryLifecycleSocketTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testGeneratedBridgeMasksMissingExecutable() throws {
        ...
        let installer = LifecycleHookInstaller(
            homeDirectory: home,
            executablePath: home.appendingPathComponent("missing-skerry").path
        )
        try installer.install(agents: [.opencode])

        let result = try run(
            executable: home.appendingPathComponent(".skerry/bin/skerry-hook"),
    ...
    func testGeneratedBridgeMasksRefusedSocket() throws {
        ...
        let path = home.appendingPathComponent(".skerry/lifecycle.sock").path
    ...
    func testGeneratedBridgeMasksWriteBackpressure() throws {
        let home = try makeBridgeRuntimeHome()
        let peer = try Peer(
            path: home.appendingPathComponent(".skerry/lifecycle.sock").path,
            behavior: .neverReads
        )
        defer { peer.stop() }
    ...
    func testActualLifecycleEntryPointIsBoundedAndFailOpen() throws {
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: home.appendingPathComponent(".skerry").path))
    ...
    private func makeBridgeRuntimeHome() throws -> URL {
        ...
        try FileManager.default.createDirectory(
            at: home.appendingPathComponent(".skerry", isDirectory: true),
    ...
    private func assertGeneratedBridgeFailsOpen(home: URL, input: Data) throws {
        ...
        try installer.install(agents: [.opencode])
        let result = try run(
            executable: installerHome.appendingPathComponent(".skerry/bin/skerry-hook"),
            arguments: ["codex", "started"],
            input: input,
