...
final class LifecycleHookInstallerTests: XCTestCase {
    ...
    private let executable = "/Applications/Skerry.app/Contents/MacOS/Skerry"

    override func setUpWithError() throws {
        home = FileManager.default.temporaryDirectory.appendingPathComponent("SkerryHookInstallerTests-\(UUID().uuidString)")
    ...
    func testInstallsEverySupportedIntegrationIdempotently() throws {
        ...
        let legacyOpenCodePlugin = home.appendingPathComponent(".config/opencode/plugin/skerry.js")
        ...
        let bridgeURL = home.appendingPathComponent(".skerry/bin/skerry-hook")
        ...
        let pi = try String(contentsOf: home.appendingPathComponent(".pi/agent/extensions/skerry.ts"), encoding: .utf8)
        ...
        let openCode = try String(
            contentsOf: home.appendingPathComponent(".config/opencode/plugins/skerry.js"),
            encoding: .utf8
        )
    ...
    func testRepairMigratesEveryOwnedAtollIntegrationAndPreservesUnrelatedConfiguration() throws {
        ...
        XCTAssertTrue(
            FileManager.default.fileExists(
                atPath: home.appendingPathComponent(".pi/agent/extensions/skerry.ts").path
            )
        )
        XCTAssertTrue(
            FileManager.default.fileExists(
                atPath: home.appendingPathComponent(".config/opencode/plugins/skerry.js").path
            )
        )
        for agent in LifecycleHookInstaller.supportedAgents {
    ...
    func testSupportedHarnessesAreExactlyTheFiveShippedAgentsInDisplayOrder() {
        ...
        XCTAssertEqual(Set(AgentHarness.allCases), Set(expected).union([.skerry]))
    ...
    func testManagedPluginCollisionIsNotOverwritten() throws {
        let plugin = home.appendingPathComponent(".config/opencode/plugins/skerry.js")
    ...
    func testUnownedLegacyPluginIsPreserved() throws {
        let plugin = home.appendingPathComponent(".config/opencode/plugin/skerry.js")
        try FileManager.default.createDirectory(at: plugin.deletingLastPathComponent(), withIntermediateDirectories: true)
        try "// another plugin".write(to: plugin, atomically: true, encoding: .utf8)
    ...
    func testDetectionAndCustomUserHomesCoverAllDocumentedOverrides() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: pi.appendingPathComponent("extensions/skerry.ts").path))
        XCTAssertTrue(FileManager.default.fileExists(atPath: openCode.appendingPathComponent("plugins/skerry.js").path))
    }
    ...
    func testPiVersionGateAndManagedFileRepair() throws {
        ...
        let managedFiles: [(AgentHarness, String)] = [
            (.pi, ".pi/agent/extensions/skerry.ts"),
            (.opencode, ".config/opencode/plugins/skerry.js")
        ]
    ...
    func testEveryIntegrationRequiresTheCurrentExecutableBridge() throws {
        let installer = makeInstaller()
        let bridge = home.appendingPathComponent(".skerry/bin/skerry-hook")
    ...
    func testFirstSharedConfigurationBackupIsExactPrivateAndNeverOverwritten() throws {
        ...
        let backupDirectory = home.appendingPathComponent(".skerry/backups/live-status")
    ...
    func testTargetedUninstallSubtractsEveryOwnedCommandAndPreservesUserChanges() throws {
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: home.appendingPathComponent(".skerry/bin/skerry-hook").path))
    }
    ...
    func testInstallUpdateUninstallAndReinstallAllProviders() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: home.appendingPathComponent(".skerry/bin/skerry-hook").path))
...
private final class BackupMutationFileManager: FileManager {
    ...
    override func setAttributes(_ attributes: [FileAttributeKey: Any], ofItemAtPath path: String) throws {
        ...
        if !didMutate, path.contains("/.skerry/backups/live-status") {
            didMutate = true
