...
final class LifecycleEventTests: XCTestCase {
    func testRegistrySkipsPersistedRecordsForRemovedHarnesses() throws {
        let fileURL = FileManager.default.temporaryDirectory.appendingPathComponent("TopsideLossyRegistry-\(UUID().uuidString).json")
    ...
    override func setUpWithError() throws {
        directory = FileManager.default.temporaryDirectory.appendingPathComponent("TopsideLifecycleTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testNormalizesHookStateWithoutRetainingSensitiveContent() throws {
        ...
        XCTAssertEqual(event.label, "Topside-Privacy")
    ...
    func testQueueAndRegistryPersistOnlyPrivacySafeLifecycleMetadata() throws {
        ...
        let queueDirectory = directory.appendingPathComponent(".topside/lifecycle-events", isDirectory: true)
    ...
    func testRegistryPersistsTerminalEventAndExpiresStaleActiveSession() throws {
        ...
        registry.ingest(LifecycleEvent(sessionID: "one", harness: .codex, kind: .started, timestamp: start, label: "Topside-Privacy"), now: start)
        ...
        XCTAssertEqual(reloaded.sessions(now: start).first?.label, "Topside-Privacy")
        XCTAssertEqual(
            reloaded.ingest(
    ...
    func testQueueRejectsInternalHarnessAndRemovesLegacyRecord() throws {
        let queue = LifecycleEventQueue(homeDirectory: directory)
        XCTAssertNil(queue.enqueue(LifecycleEvent(sessionID: "internal", harness: .topside, kind: .started)))

        let queueDirectory = directory.appendingPathComponent(".topside/lifecycle-events", isDirectory: true)
        try FileManager.default.createDirectory(at: queueDirectory, withIntermediateDirectories: true)
        let legacyFile = queueDirectory.appendingPathComponent("internal.json")
        let event = LifecycleEvent(sessionID: "internal", harness: .topside, kind: .started)
