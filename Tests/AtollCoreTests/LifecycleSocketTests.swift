...
final class LifecycleSocketTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("AtollLifecycleSocketTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testDeliveryFallsBackOnceToQueueAndIgnoresQueueFailure() throws {
        ...
        try Data("blocked".utf8).write(to: blockedHome.appendingPathComponent(".atoll"))
    ...
    func testGeneratedBridgeMasksMissingExecutable() throws {
        ...
        let installer = LifecycleHookInstaller(
            homeDirectory: home,
            executablePath: home.appendingPathComponent("missing-atoll").path
        )
        try installer.install(agents: [.opencode])

        let result = try run(
            executable: home.appendingPathComponent(".atoll/bin/atoll-hook"),
    ...
    func testGeneratedBridgeMasksRefusedSocket() throws {
        ...
        let path = home.appendingPathComponent(".atoll/lifecycle.sock").path
    ...
    func testGeneratedBridgeMasksWriteBackpressure() throws {
        let home = try makeBridgeRuntimeHome()
        let peer = try Peer(
            path: home.appendingPathComponent(".atoll/lifecycle.sock").path,
            behavior: .neverReads
        )
        defer { peer.stop() }
    ...
    func testActualLifecycleEntryPointIsBoundedAndFailOpen() throws {
        ...
        XCTAssertFalse(FileManager.default.fileExists(atPath: home.appendingPathComponent(".atoll").path))
    ...
    private func makeBridgeRuntimeHome() throws -> URL {
        ...
        try FileManager.default.createDirectory(
            at: home.appendingPathComponent(".atoll", isDirectory: true),
    ...
    private func assertGeneratedBridgeFailsOpen(home: URL, input: Data) throws {
        ...
        try installer.install(agents: [.opencode])
        let result = try run(
            executable: installerHome.appendingPathComponent(".atoll/bin/atoll-hook"),
            arguments: ["codex", "started"],
            input: input,
